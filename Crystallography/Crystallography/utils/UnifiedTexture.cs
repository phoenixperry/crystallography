...
namespace Crystallography.Utils
{
...
//		public TextureInfo GetTextureInfo (UnifiedTextureInfo unifiedInfo)
//		{
//			Sce.PlayStation.Core.Graphics.Texture2D texture;
//			TRS trs = new TRS(new Bounds2(new Vector2(unifiedInfo.u0, unifiedInfo.v0), new Vector2(unifiedInfo.u1, unifiedInfo.v1)));
//			TextureInfo info = new TextureInfo(texture, new Vector2i(1,1), trs);
//			return info;
//		}
	}
...
//					y=float.Parse(element.Attribute("y").Value);
					y=textureHeight - (h + float.Parse(element.Attribute("y").Value));	// PSM sprite origins are in lower left, not upper left
